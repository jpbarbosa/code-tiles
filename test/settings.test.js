...
import { seamSettings, settingsFrom } from '../src/guest/disk/settings.js';
...
test('the seams win the merge, and survive a source that cannot be read', () => {
  const wanted = seamSettings();
  const key = Object.keys(wanted)[0];
  const file = source(JSON.stringify({ [key]: '__the desktop value__', 'editor.fontSize': 20 }));

  const merged = JSON.parse(settingsFrom(file));
  assert.equal(merged[key], wanted[key]);
  assert.equal(merged['editor.fontSize'], 20);

  assert.deepEqual(JSON.parse(settingsFrom(path.join(os.tmpdir(), 'ct-absent.json'))), wanted);
  assert.deepEqual(JSON.parse(settingsFrom(null)), wanted);
});
